...
<!-- <img alt="image" class="img-responsive" src="../img/Mada-Denim-Blanco4.jpg"> -->
    ...
    <body>
      ...
      @foreach($condominio as $co)
          <tbody>
            ...
            <?php 
            if($cuenta->concepto == 'Todas'){
               $factura = \SIGRECOFERO\facturacion::where('emision','=','Emitido')->where('mes','=',$cuenta->mes)->where('ano','=',$cuenta->ano)->get();
               
             }else{
              $factura = \SIGRECOFERO\facturacion::where('concepto','=',$cuenta->concepto)->where('emision','=','Emitido')->where('mes','=',$cuenta->mes)->where('ano','=',$cuenta->ano)->where('id_Condominio','=',$co->id)->get();
            ...
            if($j < $count ){         
            ?>
            @if($factura[$j]->concepto == 'Administrativo')
            <?php 
            
            //dd($factura[$j]->concepto);
            ...
            <td style = "width:30%">{{'Cuota '.$factura[$j]->concepto}}</td>
    ...
    <div class="box-body">
            ...
            <tbody>
              ...
              <?php 
              if($cuenta->concepto == 'Todas'){
               $factura = \SIGRECOFERO\facturacion::where('emision','=','Emitido')->where('mes','=',$cuenta->mes)->where('ano','=',$cuenta->ano)->where('id_Condominio','=',$co->id)->get();
             }else{
              $factura = \SIGRECOFERO\facturacion::where('concepto','=',$cuenta->concepto)->where('emision','=','Emitido')->where('mes','=',$cuenta->mes)->where('ano','=',$cuenta->ano)->where('id_Condominio','=',$co->id)->get();
             }
             if(!$factura->isEmpty()){
